// Package backend wires the linapro-extid-core source plugin into the host
...
import (
	...
	pluginextidcore "lina-plugin-linapro-extid-core"
	"lina-plugin-linapro-extid-core/backend/cap/extidcap"
	handoffctrl "lina-plugin-linapro-extid-core/backend/internal/controller/handoff"
	identityctrl "lina-plugin-linapro-extid-core/backend/internal/controller/identity"
	handoffsvc "lina-plugin-linapro-extid-core/backend/internal/service/handoff"
	identitysvc "lina-plugin-linapro-extid-core/backend/internal/service/identity"
	catalogsvc "lina-plugin-linapro-extid-core/backend/internal/service/providercatalog"
)

// pluginID is the immutable identifier declared by the embedded plugin.yaml.
const pluginID = "linapro-extid-core"
...
// init registers the linapro-extid-core source plugin, declares the
...
func init() {
	...
	plugin.Assets().UseEmbeddedFiles(pluginextidcore.EmbeddedFiles)
...
func sharedIdentityDomain(users usercap.Service) (*identitysvc.Service, error) {
	if users == nil {
		return nil, gerror.New("linapro-extid-core identity domain requires host user capability")
	}
	identityMu.Lock()
...
// newProvider creates the linapro-extid-core external-identity provider from
...
func registerRoutes(_ context.Context, registrar pluginhost.HTTPRegistrar) error {
	...
	if services == nil || services.Users() == nil || services.BizCtx() == nil {
		return gerror.New("linapro-extid-core routes require host user and bizctx capabilities")
	}
