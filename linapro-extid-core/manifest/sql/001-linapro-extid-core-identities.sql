...
COMMENT ON TABLE user_external_identity IS 'Verified external identity to local user linkage owned by linapro-extid-core';
...
COMMENT ON COLUMN user_external_identity."subject_kind" IS 'Subject classification: oidc_sub, openid, unionid, custom';
COMMENT ON COLUMN user_external_identity."app_context" IS 'Multi-app context such as WeChat appId or Douyin clientKey';
...
COMMENT ON COLUMN user_external_identity."phone_snapshot" IS 'Phone captured at link time for audit only';
COMMENT ON COLUMN user_external_identity."display_name_snapshot" IS 'Display name captured at link time';
COMMENT ON COLUMN user_external_identity."avatar_url_snapshot" IS 'Avatar URL captured at link time';
