...
export const pluginPageMeta = {
  ...
  title: 'plugin:linapro-oidc-discord:settings',
};
</script>
...
<template>
  <div style="max-width: 720px; margin: 0 auto; padding: 24px">
    ...
    <ACard :title="$t('plugins.oidc.settings.cardTitle', { provider: $t('plugins.oidc.discord.displayName') })">
      <AForm class="space-y-5">
        <AFormItem>
          <div class="mb-2 font-medium">{{ $t('plugins.oidc.settings.clientIdLabel') }}</div>
          <AInput v-model:value="form.clientId" :placeholder="$t('plugins.oidc.settings.clientIdPlaceholder', { provider: $t('plugins.oidc.discord.displayName') })" />
          <div class="mt-1 text-xs text-gray-500">{{ $t('plugins.oidc.settings.clientIdHelp', { console: $t('plugins.oidc.discord.consoleName') }) }}</div>
        </AFormItem>

        <AFormItem>
          <div class="mb-2 font-medium">{{ $t('plugins.oidc.settings.clientSecretLabel') }}</div>
          <AInputPassword v-model:value="form.clientSecret" :placeholder="$t('plugins.oidc.settings.clientSecretPlaceholder', { provider: $t('plugins.oidc.discord.displayName') })" />
        ...
        <AFormItem>
          ...
          <div class="mt-1 text-xs text-gray-500">{{ $t('plugins.oidc.settings.redirectUriHelp', { section: $t('plugins.oidc.discord.registerSection') }) }}</div>
        </AFormItem>
