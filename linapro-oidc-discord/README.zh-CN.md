...
- 在 `frontend/slots/auth.login.after/discord-login-entry.vue` 提供一个 Vue 插槽组件，前端 slot registry 会在插件安装并启用时自动挂载到登录表单下方。
...
linapro-oidc-discord/
  ...
  frontend/
    slots/auth.login.after/       "使用 Discord 账号登录" 按钮组件
  manifest/
    i18n/en-US/                   英文语言包
    i18n/zh-CN/                   简体中文语言包
  README.md
...
Vue 插槽组件位于 `frontend/slots/auth.login.after/discord-login-entry.vue`。构建期由 slot registry 自动发现；运行期会在插件未安装或已禁用时隐藏按钮。按钮点击时执行整页跳转，以便浏览器保留插件写入的 Cookie。
