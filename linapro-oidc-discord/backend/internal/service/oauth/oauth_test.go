// This file verifies the Discord OAuth state-token signing helpers used by
// the authorization callback. The roundtrip and tamper-resistance behavior
// are covered without any network access so the tests are stable in CI.
...
func TestStateRejectsTamperedSignature(t *testing.T) {
	token, err := encodeState(StatePayload{StateKey: "discord", Nonce: "nonce"}, "client-secret")
	if err != nil {
		t.Fatalf("encode oauth state failed: %v", err)
	}
...
func TestStateRejectsDifferentSecret(t *testing.T) {
	token, err := encodeState(StatePayload{StateKey: "discord", Nonce: "nonce"}, "old-secret")
	if err != nil {
		t.Fatalf("encode oauth state failed: %v", err)
	}
