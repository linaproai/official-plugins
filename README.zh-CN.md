...
| `linapro-oidc-google` | `source` | `platform_only` | `global` | 基于宿主外部身份 seam 的 Google OIDC 第三方登录参考实现；依赖 `linapro-extid-core` |
| `linapro-oidc-discord` | `source` | `platform_only` | `global` | 基于宿主外部身份 seam 的 Discord OIDC 第三方登录参考实现；依赖 `linapro-extid-core` |
