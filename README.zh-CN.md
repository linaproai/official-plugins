...
| `linapro-oidc-google` | `source` | `platform_only` | `global` | Google / Gmail `OAuth2`登录入口与`OIDC` provider 元数据 |
| `linapro-oidc-discord` | `source` | `platform_only` | `global` | Discord `OAuth2`登录入口与`OIDC` provider 元数据 |
