  ...
  test('TC-1a: 供应商列表可查看模型维护结果', async ({ adminPage }) => {
    await withAdminApi(async (api) => {
      ...
      try {
        ...
        await expect(adminPage.getByText(fixture.providerName)).toBeVisible();
        const models = await listProviderModels(api, fixture.providerId);
        expect(models.some((item: { modelName?: string }) => item.modelName === fixture.modelName)).toBe(true);
      } finally {
  ...
  test('TC-1b: 编辑供应商名称', async ({ adminPage }) => {
    await withAdminApi(async (api) => {
      ...
      try {
        ...
        await expect(adminPage.getByText(renamedProviderName)).toBeVisible();
      } finally {
        await deleteProvider(api, fixture.providerId).catch(() => {});
      }
    });
