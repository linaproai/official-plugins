...
type GetSettingsReq struct {
	g.Meta `path:"/plugin/linapro-oidc-google/settings" method:"get" tags:"Google OIDC" summary:"Get Google OIDC settings" dc:"Retrieve the current Google OIDC login plugin configuration including client ID and redirect URI. The client secret is masked in the response. Provider enablement is controlled by host plugin status." permission:"linapro-oidc-google:settings:view"`
}
...
type GetSettingsRes struct {
	...
	DefaultBackendRedirect string `json:"defaultBackendRedirect" dc:"Default redirect URL after login when state does not match any rule" eg:"/dashboard/analytics"`
	BackendRedirects       string `json:"backendRedirects" dc:"JSON mapping of state to redirect URL, e.g. {\"dashboard\":\"/dashboard/analytics\",\"profile\":\"/dashboard/profile\"}" eg:"{\"dashboard\":\"/dashboard/analytics\"}"`
}
