// This file verifies Google OIDC configuration persistence rules against an
// in-memory PluginSettingsService double so callers do not need a live
...
// the previously stored secret intact. This is the "leave empty to keep
// current" contract that the admin UI relies on after masking the secret.
...
// GetInt returns the parsed int value with default fallback (parsing
// failures resolve to the default to mirror the real service behavior).
