...
//
// Splitting persistence into the host service has three benefits:
//   - the plugin no longer needs a private SQL table, migration, or seed file;
//   - schema upgrades stop requiring per-plugin ALTER TABLE statements;
//   - the operator audits every plugin's configuration in one sys_config UI.
package config
...
// package owns. It is exported so the controller, provider, and tests stay
// in lockstep without hard-coding the string.
const PluginID = "linapro-oidc-google"

// Settings is the typed projection of the Google OIDC plugin configuration.
// It is the value shape the controller exchanges with admin UIs and the
// provider consults when assembling login entry metadata.
type Settings struct {
	// ClientID is the Google OAuth2 client identifier.
	ClientID string
	// ClientSecret is the Google OAuth2 client secret. Empty values returned
	// by Get mean "no secret stored". Empty values passed to Save mean
	// "preserve the stored secret unchanged".
	ClientSecret string
	// RedirectURI is the OAuth2 redirect URI registered in Google Cloud
	// Console.
	RedirectURI string
...
// namespace. The values are stable contracts because they appear in the
// sys_config.key column and in operator audit logs.
const (
...
// Get returns the current Google OIDC settings, applying built-in defaults
// for missing values.
func (s *Service) Get(ctx context.Context) (*Settings, error) {
	if s == nil || s.settings == nil {
		return nil, gerror.New("google oidc settings service is not initialized")
	}
...
// Save persists the supplied Google OIDC settings.
//
// ClientSecret follows the secret-style "leave empty to preserve" convention
// because admin UIs mask the stored secret on read; if the operator leaves
// the field blank we must not overwrite the persisted secret with an empty
// value. Every other field is treated as a plain string with empty values
// clearing the stored row so subsequent reads return the configured default.
func (s *Service) Save(ctx context.Context, in *Settings) error {
	if s == nil || s.settings == nil {
		return gerror.New("google oidc settings service is not initialized")
	}
	if in == nil {
		return gerror.New("google oidc settings save requires non-nil payload")
	}
