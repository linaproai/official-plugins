...
func (s *serviceImpl) BuildAuthorizeURL(ctx context.Context, stateKey string) (AuthorizeRequest, error) {
	// Re-resolve settings per request so admin edits apply without a restart.
	config := s.resolveConfig(ctx)
	if strings.TrimSpace(config.ClientID) == "" || strings.TrimSpace(config.RedirectURL) == "" || strings.TrimSpace(config.AuthorizeURL) == "" {
	...
	state, err := s.stateCodec.Encode(ctx, stateKey, config.ClientSecret)
	if err != nil {
