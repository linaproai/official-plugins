// Package identity implements the linapro-oidc-core external-identity provider:
...
import (
	...
	"lina-plugin-linapro-oidc-core/backend/internal/model/entity"
)
...
// Service implements the externalidentityspi.Provider contract on top of the
// plugin-owned user_external_identity table and the host user capability.
type Service struct {
	users usercap.Service
}

// Ensure Service implements the host external-identity provider SPI.
var _ externalidentityspi.Provider = (*Service)(nil)

// New creates the linapro-oidc-core external-identity provider from the
// injected host user capability.
func New(users usercap.Service) (*Service, error) {
	if users == nil {
		return nil, gerror.New("linapro-oidc-core identity provider requires host user capability")
	}
...
// outcome.
func (s *Service) Resolve(ctx context.Context, in externalidentityspi.ResolveInput) (int, bool, error) {
...
func (s *Service) Provision(ctx context.Context, in externalidentityspi.ProvisionInput) (int, error) {
	provider := strings.TrimSpace(in.Provider)
	subject := strings.TrimSpace(in.Subject)
	if provider == "" || subject == "" {
...
func (s *Service) Bind(ctx context.Context, in externalidentityspi.BindInput) error {
	provider := strings.TrimSpace(in.Provider)
	subject := strings.TrimSpace(in.Subject)
...
func (s *Service) Unbind(ctx context.Context, in externalidentityspi.UnbindInput) error {
	provider := strings.TrimSpace(in.Provider)
	subject := strings.TrimSpace(in.Subject)
	if in.UserID <= 0 || provider == "" || subject == "" {
		return bizerr.NewCode(CodeIdentityInvalid)
...
func (s *Service) List(ctx context.Context, userID int) ([]externalidentityspi.BoundIdentity, error) {
	if userID <= 0 {
		return []externalidentityspi.BoundIdentity{}, nil
	}
	...
	identities := make([]externalidentityspi.BoundIdentity, 0, len(rows))
	for _, row := range rows {
		if row == nil {
			continue
		}
		identities = append(identities, externalidentityspi.BoundIdentity{
...
// provisionAccount mints one least-privilege host account through the user
// capability and parses the returned domain ID. Same-email conflict policy:
// the login path carries no actor context, so the unfiltered email-existence
// check is enforced by the host minting primitive itself; this provider maps
// the capability sentinel into its caller-visible conflict code, rejecting
// silent auto-provisioning so the user signs in to the existing account and
// binds the identity explicitly instead of enabling an email-assertion
// takeover.
func (s *Service) provisionAccount(ctx context.Context, provider string, email string, anchor string, displayName string) (int, error) {
	provisionedID, err := s.users.ProvisionExternal(ctx, usercap.ProvisionExternalInput{
		Email:          email,
	...
	if err != nil {
		if errors.Is(err, usercap.ErrProvisionEmailConflict) {
