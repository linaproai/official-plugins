...
type UnbindIdentityReq struct {
	g.Meta   `path:"/identities" method:"delete" tags:"External Identity" summary:"Unbind an external identity" dc:"Removes one external identity linkage from the current session user. Cross-user targets report not found without leaking existence."`
